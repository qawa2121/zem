#!/bin/bash
#Fix/Edit ' GaSXV2#2605
echo "Enter Main Tor Address:"
read TORDOMAINMY
#echo "Enter Password:"
#read password
...
mysql -uroot --password="$password" -e "CREATE USER 'cerberusroot'@'localhost' IDENTIFIED BY '$password';"
mysql -uroot --password="$password" bot < dfsql.sql
mysql -uroot --password="$password" -e "GRANT ALL PRIVILEGES ON *.* TO 'cerberusroot'@'localhost';"
